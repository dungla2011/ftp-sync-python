...
fileConfig = os.path.join(os.path.expanduser("~"), "ftp_upload_glx.conf")
fileLog = os.path.join(os.path.expanduser("~"), "ftp_upload_glx.log")
    ...
    while True:
        ...
        try:
            ...
            if 1:
                ...
                for root, subdirectories, files in os.walk(ftpSetting.setFolderLocal):
                    for subdirectory in subdirectories: #build subfolders server folder if they dont exist
                        
                        required_dir = os.path.join(root, subdirectory).replace(ftpSetting.setFolderLocal, '').split('\\')[1:]
                        #returns an array of folders to add e.g. required_dir = ['folder0', 'folder1', 'folder2']
                    ...
                    for fileUp in files:
                        ...
                        fileUpName = fileUp
                        fileUp = os.path.join(root, fileUp)  
                        #navigate to folder file is in on the server e.g fileUp_elements = ['folder0', 'folder1', 'folder2']
                        fileUp_elements = fileUp.replace(ftpSetting.setFolderLocal, '').replace(fileUpName, "").split('\\')[1:]
                        ftp.cwd(ftpSetting.setFolderServer)
                        for folder in fileUp_elements:
                            ftp.cwd(folder)
                            ...
                            with open(fileUp, 'rb') as f:
                                ...
                                if os.path.basename(fileUp) in files_list:
                                    ...
                                    try:
                                        ...
                                        ftp.storbinary('STOR ' + os.path.basename(fileUp), f, blocksize=blockSizeUpload, callback=uploadTracker.handle, rest=rest_pos)
                                    ...
                                    #      flush=True)
                                    ftp.storbinary('STOR ' + os.path.basename(fileUp), f, blockSizeUpload, uploadTracker.handle)
                                    ol1("Upload done " + fileUp)
                                    done = True
